...
export default function ClaimActions({ claimId, businessId, userId }: { claimId: string; businessId: string; userId: string }) {
  ...
  const handleAction = async (action: 'approve' | 'reject') => {
    ...
    await fetch('/api/admin/claims', {
      ...
      body: JSON.stringify({ claimId, businessId, userId, action, note }),
    })
    setLoading(false)
    router.refresh()
  }
