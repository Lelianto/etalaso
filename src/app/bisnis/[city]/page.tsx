...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `Temukan bisnis lokal di ${cityName}. Lihat profil lengkap dan hubungi langsung via WhatsApp.`,
  }
}
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        ...
        {sortedCategories.map((category) => (
          <section key={category} className="mb-12">
            <h2 className="font-semibold text-lg text-charcoal mb-4 capitalize">{category}</h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {grouped[category].map((biz) => {
                const slug = toSlug(biz.name)
                const catSlug = toSlug(biz.category || 'bisnis')
                return (
                  <Link
                    key={biz.name}
